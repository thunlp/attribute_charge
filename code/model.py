...
class LSTM_MODEL(object):
	...
	def __init__(self, word_embeddings, attr_table, config):
		...
		#attr table contains pre-labelled attributes for each article
		#attr_table's size = [num of articles, num of attributes]
		self.attr_table = tf.Variable(attr_table, trainable = False)
		
		#input_attr_mask is used to mask loss from input with attribute of "NA"
		...
		#use lstm to get hidden state sequence
		...
		#for each attribute, get the attribute-aware representation of fact.
		...
		#use max pooling layer to get the attribute-free representation of the fact
		...
		#concat the averaged attribute-aware representation and the attribute-free representation
		#use the final representation to predict the charge
		with tf.name_scope("lstm_output"):
			softmax_w = tf.get_variable("softmax_w", [size+size, num_classes])
			softmax_b = tf.get_variable("softmax_b", [num_classes])	
			ave_attention_outputs = tf.reduce_sum(tf.concat(1, [tf.expand_dims(temp,1) for temp in self.attention_outputs]),axis=1)
			temp = [self.output]
	...
	def attention(self,inputs,attention_size):
		...
		#for each attribute, use attention layer to get the attribute-aware representation of the fact
